<template>
  ...
  infinite-scroll-disabled="loading" infinite-scroll-distance="10">
      ...
      <div class="item" v-for="(item,index) in orderList" @click="goDetail(item.orderId)">
        ...
        <!--<span v-if="isOrderTime(item.eatStyle,item.isSupportDelivery)">下单时间&nbsp;&nbsp;{{item.createTimeStr}}</span>-->
        <div class="detail">
            <p v-if="item.eatStyle == 3 || item.eatStyle == 4">{{item.createTimeStr}}</p>
            <p>
              <span v-if="isGetTime(item.eatStyle,item.isSupportDelivery)">取餐时间</span><span v-if="isEatTime(item.eatStyle,item.isSupportDelivery)">就餐时间</span><span v-if="isOrderTime(item.eatStyle,item.isSupportDelivery)">下单时间</span>&nbsp;&nbsp;<span v-if="item.eatStyle ==1 || item.eatStyle ==2">{{item.createTimeStr}}</span><span v-if="item.eatStyle ==3 || item.eatStyle ==4">{{item.eatTimeStr}}</span>
            </p>
            <p><span v-if="isGetNumber(item.status,item.eatStyle)">取餐号码</span><span v-if="isEatNumber(item.status,item.eatStyle)">就餐号码</span>&nbsp;&nbsp;<span>{{item.eatNumber}}</span></p>
...
<script>
  ...
  export default {
    ...
    methods : {
     ...
     isGetTime(eatStyle,isSupportDelivery){//取餐时间
       if (isSupportDelivery == 2) { //取餐模式
        if (eatStyle == 3 || eatStyle == 4) {
          return true
        } else {
          return false
        }
    ...
    isEatTime(eatStyle,isSupportDelivery){ //就餐时间
      if (isSupportDelivery == 1) { //送餐模式
        if (eatStyle == 3) {
          return true
        } else {
          return false
     ...
     isOrderTime(eatStyle,isSupportDelivery){ //下单时间
     ...
     isGetNumber(eatStyle,isSupportDelivery){//取餐号码
      if (isSupportDelivery == 2) { //取餐模式
        if (eatStyle == 3 || eatStyle == 4) {
          return true
        } else {
     ...
     isEatNumber(eatStyle,isSupportDelivery){ //就餐号码
      if (isSupportDelivery == 1) { //送餐模式
        if (eatStyle == 1|| eatStyle == 2 || eatStyle == 3) {
          return true
        } else {
          return false
        }
